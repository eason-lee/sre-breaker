...
func NewBreaker(opts ...Option) Breaker {
	...
	b.throttle = newLoggedThrottle(b.name, newSreBreaker())
...
func (lt loggedThrottle) logError(err error) error {
	if err == ErrServiceUnavailable {
		...
		log.Printf("proc(%s/%d), callee: %s, breaker is open and requests dropped\nlast errors:\n%s",
			utils.ProcessName(), utils.Pid(), lt.name, lt.errWin)
	}
...
func (ew *errorWindow) add(reason string) {
	...
	ew.count = utils.MinInt(ew.count+1, numHistoryReasons)
	ew.lock.Unlock()
}
